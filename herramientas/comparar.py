...
leer = lambda n: {(r["capa"], r["tabla"]): int(r["filas"]) for r in csv.DictReader(open(EV / f"conteos-corrida-{n}.csv", encoding="utf-8"))}
c1, c2 = leer(a), leer(b)
filas = [(k[0], k[1], c1.get(k), c2.get(k), c1.get(k) == c2.get(k)) for k in sorted(set(c1) | set(c2))]
with open(EV / "comparacion-corridas.csv", "w", newline="", encoding="utf-8") as f:
