...
Imagen: `docs/1.4-diagrama-modelo.png` (también en `.svg`; fuente en `.dot`). La misma relación en formato Mermaid, que GitHub dibuja solo:

```mermaid
erDiagram
    dim_fecha   ||--o{ dim_tiempo : "fecha_key"
    dim_sistema ||--o{ dim_estacion : "sistema_key"
    dim_zona    ||--o{ dim_estacion : "zona_key"

    dim_tiempo   ||--o{ fct_abordaje : "tiempo_key"
    dim_fecha    ||--o{ fct_abordaje : "fecha_key"
    dim_usuario  ||--o{ fct_abordaje : "usuario_sk"
    dim_sistema  ||--o{ fct_abordaje : "sistema_key"
    dim_estacion ||--o{ fct_abordaje : "estacion_key"
    dim_zona     ||--o{ fct_abordaje : "zona_key"

    dim_tiempo   ||--o{ fct_trayecto_metroriel : "tiempo_key"
    dim_fecha    ||--o{ fct_trayecto_metroriel : "fecha_key"
    dim_usuario  ||--o{ fct_trayecto_metroriel : "usuario_sk"
    dim_estacion ||--o{ fct_trayecto_metroriel : "origen / destino"
    dim_zona     ||--o{ fct_trayecto_metroriel : "origen / destino"

    dim_fecha    ||--o{ fct_padron_diario : "fecha_key"
    dim_zona     ||--o{ fct_padron_diario : "zona_key (residencia)"

    fct_abordaje {
        bigint abordaje_key PK
        varchar abordaje_id_origen "degenerada"
        varchar tipo_validacion "degenerada"
        bigint tiempo_key FK
        int fecha_key FK
        varchar usuario_sk FK
        bigint sistema_key FK
        bigint estacion_key FK
        int zona_key FK
        int cantidad_abordajes "aditiva"
        decimal monto_q "aditiva"
    }
    fct_trayecto_metroriel {
        bigint trayecto_key PK
        bigint estacion_origen_key FK
        bigint estacion_destino_key FK
        int zona_origen_key FK
        int zona_destino_key FK
        int cantidad_trayectos "aditiva"
        double duracion_min "aditiva"
        int estaciones_recorridas "aditiva"
        decimal monto_q "aditiva"
    }
    fct_padron_diario {
        int fecha_key PK
        int zona_key PK
        varchar perfil PK
        varchar operador_tarjeta PK
        int tarjetas_activas "semi-aditiva"
        int altas_del_dia "aditiva"
        int bajas_del_dia "aditiva"
    }
    dim_tiempo {
        bigint tiempo_key PK "yyyymmddhh"
        int hora
        bool es_dia_habil
        bool es_hora_pico
    ...
    dim_zona {
        int zona_key PK
        varchar tipo_territorio "zona o municipio"
        bool tiene_servicio
        int modos_con_estacion
        bool en_trazado_metroriel
    }
    dim_usuario {
        varchar usuario_sk PK "seudónimo"
        int sistemas_vinculados
        bool es_multisistema
    }
```
